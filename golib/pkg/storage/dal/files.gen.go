	...
	_file.Headlines = fileHasManyHeadlines{
		...
		Tags: struct {
			...
			Headline: struct {
				...
				RelationField: field.NewRelation("Headlines.Tags.Headline", "storage.Headline"),
			},
		},
	}
...
type IFileDo interface {
	...
	Unscoped() IFileDo
	Create(values ...*storage.File) error
	CreateInBatches(values []*storage.File, batchSize int) error
	Save(values ...*storage.File) error
	First() (*storage.File, error)
	Take() (*storage.File, error)
	Last() (*storage.File, error)
	Find() ([]*storage.File, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*storage.File, err error)
	FindInBatches(result *[]*storage.File, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*storage.File) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*storage.File, error)
	FirstOrCreate() (*storage.File, error)
	FindByPage(offset int, limit int) (result []*storage.File, count int64, err error)
