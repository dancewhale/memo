...
func (api *CardApi) RemoveCard(orgid string) error {
	n := api.Card
	Card, err := n.WithContext(context.Background()).Where(n.Orgid.Eq(orgid)).First()
	if err != nil {
...
func (api *CardApi) AddReviewLog(orgid string, rlog *gfsrs.ReviewLog) error {
	...
	n := api.Card
	Card, err := n.WithContext(context.Background()).Preload(n.ReviewLogs).Where(n.Orgid.Eq(orgid)).First()
	if err != nil {
...
func (api *CardApi) DueCardsInDay(day int64) []*storage.Card {
	...
	cards, err := n.WithContext(context.Background()).Join(f, n.Orgid.EqCol(f.CardOrgid)).Where(f.Due.Gte(dueDayStart)).Where(f.Due.Lte(dueDayEnd)).Find()
	if err != nil {
...
func (api *CardApi) DueCardsDeferred() []*storage.Card {
	...
	cards, err := c.WithContext(context.Background()).Join(f, c.Orgid.EqCol(f.CardOrgid)).Where(f.Due.Lte(todayStart)).Find()
	if err != nil {
...
func (api *CardApi) IfCardIsDue(orgid string) (bool, error) {
	...
	Cards, err := n.WithContext(context.Background()).Join(f, n.Orgid.EqCol(f.CardOrgid)).Where(n.Orgid.Eq(orgid)).Where(f.Due.Lte(todayEnd)).Find()
	if err != nil {
...
func (api *CardApi) ScanOrgForCardInit() ([]*storage.Card, error) {
	...
	Cards, err := Card.WithContext(context.Background()).LeftJoin(fsrsInfo, fsrsInfo.CardOrgid.EqCol(Card.Orgid)).Where(fsrsInfo.ID.IsNull()).Find()
	if err != nil {
