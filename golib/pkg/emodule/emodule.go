...
func (e *EModule) GetNextReviewNote(ectx emacs.FunctionCallContext) (emacs.Value, error) {
	...
	head, err := e.hapi.GetHeadlineByOrgID(fcard.ID)
	if err != nil {
	...
	if head == nil {
		if head.Type == nil {
			...
			err = logger.Errorf("Get headline by orgid %s failed: %v", fcard.ID, err)
			return e.EmacsReturn(ectx, err, env.String(fcard.ID), env.String(*head.Type), env.String(""), env.String(""))
		}
	}
...
func (e *EModule) UploadFile(ectx emacs.FunctionCallContext) (emacs.Value, error) {
	...
	if force == "true" {
		err = e.hapi.UploadFile(filePath, true)
	} else {
		err = e.hapi.UploadFile(filePath, false)
	}
	if err != nil {
	...
	err = godirwalk.Walk(dirPath, &godirwalk.Options{
		Callback: func(osPathname string, de *godirwalk.Dirent) error {
			...
			if strings.Contains(osPathname, ".org") && !de.IsDir() {
				err = e.hapi.UploadFile(osPathname, needForce)
